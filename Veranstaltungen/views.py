...
class VeranstaltungDetail(DetailMitMenue):
    ...
    def post(self, request, *args, **kwargs):
        if request.user.is_staff:
            ...
            content = ('<h1>Teilnehmer für %s</h1><table style="text-align: left;"><tr><th>Nachname</th>'
                       '<th>Vorname</th><th>Anzahl</th><th>Email</th></tr>' % v)
            n = 0
            kaeufe = kaeufe.order_by('nutzer__user__last_name')
            for k in kaeufe:
                teilnahme = '<tr><td>%s</td><td>%s</td><td>%d</td><td>%s</td></tr>' % (k.nutzer.user.last_name,
                                                                                       k.nutzer.user.first_name,
                                                                                       k.menge, k.nutzer.user.email)
                content += teilnahme
                n += k.menge
...
def daten_einlesen(request):
    ...
    aus_alter_db_einlesen()  # TODO: Missing import
